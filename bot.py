...
CARS = {
    # Стандартные (цены переведены из EUR по курсу 1€=97₽)
    1:("Honda Civic",446_200),2:("BMW E28",475_300),3:("Ford Transit",436_500),
    4:("BMW M5 E34",562_600),5:("Mercedes W140",533_500),6:("Mazda RX-8",533_500),
    7:("Toyota AE86",2_425_000),8:("BMW M3 E36",1_164_000),9:("Audi RS4",3_880_000),
    10:("Ford Mustang",4_365_000),11:("BMW M5 E60",3_686_000),12:("Dodge Charger",2_425_000),
    13:("BMW M4 F82",7_760_000),14:("Porsche 911",18_624_000),15:("Ferrari 458",29_100_000),
    16:("Lamborghini Huracan",37_345_000),17:("Bugatti Chiron",407_400_000),
    18:("Rolls-Royce Cullinan",145_500_000),19:("Koenigsegg Agera",291_000_000),
    20:("McLaren P1",145_500_000),
    # Автосалон (₽)
...
ALL_JOBS = {
    # Гос. структуры (только основатели могут выдавать)
    "Правительство":150_000,
    "ФСБ":120_000,
    "МВД":80_000,
    "Мин.Здрав":75_000,
    "ВСРФ":60_000,
    "Цодд":45_000,
    "Режиссёр (телевидение)":65_000,
    # Гражданские (любой админ)
    "Заправщик":25_000,
    "Механик":30_000,
    "Инкассатор":40_000,
    "Курьер":22_000,
...
GOV_JOBS = {"Правительство","ФСБ","МВД","Мин.Здрав","ВСРФ","Цодд","Режиссёр (телевидение)"}
...
def info_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💼 Работа", callback_data="inf_job")],
...
@dp.callback_query(F.data == "inf_job")
async def cb_inf_job(cb: types.CallbackQuery):
    u = db.get_user(cb.from_user.id)
    if not u: await cb.answer("❌", show_alert=True); return
    job = u[7]
    if not job:
        await cb.answer("💼 Работа: нет\n\nОбратись к администратору", show_alert=True); return
    sal = ALL_JOBS.get(job, 0)
    gov = "🏛 Гос. структура" if job in GOV_JOBS else "👤 Гражданская"
    mult = " 🔥 x2" if db.has_x2(cb.from_user.id) else ""
    await cb.answer(
        f"💼 Должность: {job}\n"
        f"📂 Тип: {gov}\n"
        f"💵 Зарплата: {f(sal)}{mult}\n"
        f"⏰ Раз в 90 минут",
        show_alert=True
    )
...
@dp.message(lambda m: m.text and m.text.lower().strip() in ["список работ", "работы", "вакансии"])
async def cmd_jobs(m: types.Message):
    txt = "💼 Доступные вакансии\n\n👤 Гражданские (устроиться [название]):\n"
    for job, sal in ALL_JOBS.items():
        if job not in GOV_JOBS:
            txt += f"• {job} — {f(sal)}/зп\n"
    txt += "\n🏛 Гос. структуры — только через администратора"
    await m.answer(txt)

@dp.message(lambda m: m.text and m.text.lower().startswith("устроиться "))
async def cmd_hire(m: types.Message):
    u = db.get_user(m.from_user.id)
    if chk(u) != "ok":
        await m.answer("❌ Не зарегистрированы." if chk(u) == "no" else "⛔ Заблокированы.")
        return
    job = m.text.strip()[len("устроиться "):].strip()
    if not job:
        await m.answer("❌ Формат: устроиться [название работы]"); return
    if job in GOV_JOBS:
        await m.answer(f"⛔ «{job}» — гос. должность. Обратись к администратору.")
        return
    matched = next((j for j in ALL_JOBS if j.lower() == job.lower()), None)
    if not matched:
        await m.answer(f"❌ Работа «{job}» не найдена.\nНапиши список работ чтобы увидеть вакансии.")
        return
    db.set_job(m.from_user.id, matched)
    sal = ALL_JOBS[matched]
    await m.answer(f"✅ Вы устроились: {matched}\n💵 Зарплата: {f(sal)} раз в 90 мин.\n\nНапиши зарплата чтобы получить первую зп")
...
async def cmd_adm_job(m: types.Message):
    ...
    if job in GOV_JOBS and not fnd(m.from_user.id):
        await m.answer(f"⛔ Гос. должность «{job}» может выдать только основатель."); return
    db.set_job(tgt[0], job)
    gov_tag = " 🏛" if job in GOV_JOBS else ""
    await m.answer(f"✅ @{un} — {job}{gov_tag}")
    try: await bot.send_message(tgt[0], f"💼 Вам выдана должность: {job}")
    except: pass
